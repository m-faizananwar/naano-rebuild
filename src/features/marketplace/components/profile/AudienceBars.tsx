import { BarFills } from "@/components/motion/BarFills";
        ...
        {rows.map(([label, pct]) => (
          <li key={label} className="grid grid-cols-[8rem_1fr_2.5rem] items-center gap-2 text-sm">
            ...
            <span className="h-2 overflow-hidden rounded-full bg-muted" aria-hidden="true">
              <span data-fill className="block h-full rounded-full bg-brand" style={{ width: `${Math.min(100, pct)}%` }} />
            </span>
