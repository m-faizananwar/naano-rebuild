...
export function MatchingPromptBox({ value, onChange, onSubmit, pending, placeholder }: Props) {
  ...
  return (
    <form onSubmit={submit} className="relative rounded-2xl border bg-background p-3 shadow-xs focus-within:border-brand">
      <label htmlFor="nao-prompt" className="sr-only">
        Ask Nao to find creators
      </label>
      <Textarea
        id="nao-prompt"
        value={value}
        ...
        rows={3}
        placeholder={placeholder ?? "Ask Nao a question, or find creators…"}
      ...
      />
      <Button type="submit" size="icon" aria-label="Send to Nao" disabled={pending || !value.trim()} className="absolute right-3 bottom-3 rounded-full bg-brand text-brand-foreground hover:bg-brand/90">
