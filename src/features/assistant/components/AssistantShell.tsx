...
// with no layout shift (AssistantMount). The mic is the one live control:
// a click mounts the widget and starts listening (AssistantMount forwards it).
export function AssistantShell({ mode }: { mode: "public" | "brand" | "creator" }) {
  const width = mode === "public" ? PILL_WIDTH.public : PILL_WIDTH.app;
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-30 flex justify-center" style={{ paddingBottom: EDGE_GAP }}>
      <div className="flex flex-col items-center" style={{ width, maxWidth: "calc(100vw - 2rem)" }}>
        ...
        <div className="assistant-shell-pill assistant-glass flex items-center gap-3 rounded-full pl-4 pr-1.5" style={{ width, maxWidth: "calc(100vw - 2rem)", height: PILL_HEIGHT }}>
          ...
          <button type="button" className="assistant-shell-mic pointer-events-auto flex size-8 shrink-0 items-center justify-center rounded-full bg-muted text-foreground" aria-label="Talk to the assistant">
