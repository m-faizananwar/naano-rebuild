...
export async function parseIntent(transcript: string, role: "brand" | "creator"): Promise<{ intent: VoiceIntent; source: "model" | "grammar"; reason?: string }> {
  const fallback = (reason?: string) => ({ intent: intentSchema.parse(parseVoiceCommand(transcript)), source: "grammar" as const, reason });
  ...
  try {
    ...
    if (!result) return fallback("model returned nothing usable");
    ...
    // The reason (no payload, no key) rides along so a failing provider is visible from the client.
    return fallback(describeAiError(error));
  }
}
