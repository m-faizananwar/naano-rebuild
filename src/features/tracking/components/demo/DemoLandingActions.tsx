...
export function DemoLandingActions() {
  ...
  function track(type: "signup" | "purchase") {
    ...
    if (type === "signup") window.naano("track", "signup", { email });
    else window.naano("track", "purchase", { value: DEMO_PRICE, order_id: `demo-${Date.now()}` });
    setDone(type);
    toast.success(type === "signup" ? "naano('track', 'signup') sent" : "naano('track', 'purchase') sent");
  }
  ...
  return (
    <div className="grid gap-6 rounded-2xl border bg-background p-6">
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">Submitting calls naano(&apos;track&apos;, &apos;signup&apos;, {"{ email }"}).</p>
      </form>
      <div className="grid gap-2">
        <Button type="button" variant="outline" onClick={() => track("purchase")}>
          Buy the €{DEMO_PRICE} plan
        </Button>
        <p className="text-xs text-muted-foreground">Calls naano(&apos;track&apos;, &apos;purchase&apos;, {"{ value: 49, order_id }"}).</p>
      </div>
      {done ? (
