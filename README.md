...
| Landing hero + footer | Scroll-scrubbed video hero (`/hero/index.html` standalone, `ScrollHero` on the landing) and the ink footer (`/ink-footer/index.html` standalone, `InkFooter` on the landing) built from the specs in `docs/reference/`; the gaze-scrub footer lives at `/studio-footer` only. "The Letter" sign-up writes a `newsletter_signups` row and shows a thanks — no email is sent. `/privacy` and `/terms` are plain-language notices for the demo. |
