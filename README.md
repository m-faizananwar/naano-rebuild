...
| Landing hero + footer | Scroll-scrubbed video hero (`/hero/index.html` standalone, `ScrollHero` on the landing) and the ink footer (`/ink-footer/index.html` standalone, `InkFooter` on the landing) built from the specs in `docs/reference/`; the liquid-glass page at `/terranova/` and its card, chamfer button, `+` rules, slide-in menu and card treatment layered on the landing (`src/features/public/components/glass`); the metric-cards stage at `/performance/` and as the landing's results section (`src/features/public/components/performance`, LED-dot numbers from the seeded benchmarks); every other landing card wears the hero card's glass chrome (`GlassCard`) and, inside a `GlassSection`, bends a static backdrop — the hero still at t=2s (`public/backdrops/hero-still.jpg`, `scripts/backdrops.mjs`) blurred under a paper wash, drawn into the duplicate canvas once (≥768px); the gaze-scrub footer lives at `/studio-footer` only. "The Letter" sign-up writes a `newsletter_signups` row and shows a thanks — no email is sent. `/privacy` and `/terms` are plain-language notices for the demo. |
